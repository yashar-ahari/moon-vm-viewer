...
namespace MoonSharp.Interpreter.Execution.VM
{
	internal class ByteCode : RefIdObject
	{
		...
		public string Dump(ref int offset, bool rootChunkOnly = false)
		{
			StringBuilder sb = new StringBuilder();
			...
			for (int i = 0; i < Code.Count; i++)
			{
				if(rootChunkOnly && !flag)
				{
					if (Code[i].OpCode == OpCode.Meta && Code[i].Name.Contains("<chunk-root>"))
					{
						flag = true;
						i = i - 2;
						offset = i;
		...
		public List<Instruction> DumpMemory()
		{
			return this.Code;
		}
