...
namespace MoonSharp.Interpreter.Execution.VM
{
	sealed partial class Processor
	{
		...
		const ulong DUMP_CHUNK_MAGIC_ML = 0x1A0D2343534C4D1D;
		...
		internal int Dump(Stream stream, int baseAddress, bool hasUpvalues)
		{
			...
			using (BinaryWriter bw = new BinDumpBinaryWriter(stream, Encoding.UTF8))
			{
				...
				Console.WriteLine($"WRITE {DUMP_CHUNK_MAGIC}");
				bw.Write(DUMP_CHUNK_VERSION);
				Console.WriteLine($"WRITE {DUMP_CHUNK_VERSION}");
				bw.Write(hasUpvalues);
                Console.WriteLine($"WRITE {hasUpvalues}");
                bw.Write(meta.NumVal);
                Console.WriteLine($"WRITE {meta.NumVal}");
				...
				{
					Console.WriteLine($"WRITING SYMBOL: {sym}");
					sym.WriteBinary(bw);
				}
				foreach (SymbolRef sym in allSymbols)
				{
					Console.WriteLine($"WRITING SYMBOL ENV: {sym.Environment}");
					sym.WriteBinaryEnv(bw, symbolMap);
				}
				for (int i = 0; i <= meta.NumVal; i++)
				{
                    Console.WriteLine($"WRITING INSTRUCTION[{(baseAddress + i).ToString("x")}]: {m_RootChunk.Code[baseAddress + i]}");
		...
		internal int Undump(Stream stream, int sourceID, Table envTable, out bool hasUpvalues)
		{
			...
			using (BinaryReader br = new BinDumpBinaryReader(stream, Encoding.UTF8))
			{
				...
				if (!(headerMark == DUMP_CHUNK_MAGIC || headerMark == DUMP_CHUNK_MAGIC_ML))
