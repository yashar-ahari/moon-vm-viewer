...
namespace MoonSharp.Interpreter.Execution.VM
{
	sealed partial class Processor
	{
		...
		private DynValue Processing_Loop(int instructionPtr)
		{
			...
			try
			{
				while (true)
				{
					...
					Console.WriteLine($"IC: {instructionPtr}, OPCODE {i.OpCode}, NAME: {i.Name}, VALUE: {i.Value}");
					...
					if (canAutoYield && executedInstructions > AutoYieldCounter)
					{
						...
						this.m_Script.ProcessorDebugContext.currentInstruction = instructionPtr;
						this.m_Script.ProcessorDebugContext.state = this.State;
						this.m_Script.ProcessorDebugContext.OnProcessorRun();
		...
		private void ExecStoreLcl(Instruction i)
		{
			...
			//Console.WriteLine(symref.Index);
