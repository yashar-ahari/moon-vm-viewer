...
namespace MoonSharp.Interpreter
{
	...
	public class ScriptRuntimeException : InterpreterException
	{
		...
		public static ScriptRuntimeException BadArgumentOutOfRange(int argNum, string funcName, string minValue, string maxValue)
        {
			return new ScriptRuntimeException("bad argument out of expected range #{0} to '{1}'. Accepted values must between {2} and {3}", argNum, funcName, minValue, maxValue);
        }

		public static ScriptRuntimeException BadArgumentNilValue(int argNum, string functionName)
		{
			return new ScriptRuntimeException("bad argument nil value #{0} to '{1}'", argNum, functionName);
		}

		public static ScriptRuntimeException AttemptingToSetReadOnlyProperty(string propertyName)
        {
			return new ScriptRuntimeException("attempting to write to a read-only property '{1}'", propertyName);
        }
